...
class AircraftFlightController extends Controller
{
    ...
    public function getAircraftFlightsByDate(Request $request) {
        ...
        $date_m = Carbon::create($request->date)->format('m');
        $date_d = Carbon::create($request->date)->format('d');
        $aircraft_flights = AircraftFlight::whereMonth('date', $date_m)->whereDay('date', $date_d)->get();

        // $aircraft_flights = AircraftFlight::whereMonth('date', '02')->get();
    ...
    public function getAircraftFlightsById(Request $request, $aircraftFlightId) {
    
        $aircraft_check_flight = AircraftFlight::find($aircraftFlightId);
        $aircraft_check_flight->flight;
        $aircraft_check_flight->aircraft;
        return response()->json([
            'message' => 'success',
            'aircraft_check_flight' => $aircraft_check_flight,
        ], 200);
    }
    ...
    public function updateAircraftFlight(Request $request) {
        // Update checkin aircraftflight
        $validator = Validator::make($request->all(), [
            'type' => 'required',
            'aircraft' =>'required',
            'origin_airport_name' => 'required',
            'flight' => 'required', //flightid
        ...
        $aircraft_flight = AircraftFlight::find($request->id);
    ...
    public function indivisualTicketing(Request $request)
    {
        ...
        if ($flight_passenger->status == 'CLOSED') {
            ...
            $url = url('/login?ref='.$flight_passenger->book_reference);
            try {
                $message = $client->messages->create(
                    $receiver, // Text this number
                    [
                        'from' => $sender, // From a valid Twilio number
                        'body' => $url.' You are booked on '.$flight->airline_code.$flight->flight_number.' '.$flight_passenger->aircraftFlight->date->format('Y-m-d').' from '.$flight->origin_airport_code.' to '.$flight->destination_airport_code.'. Your booking reference is: '.$flight_passenger->book_reference.'. Web check-in is available via https://flightres.tech'
                    ]
                );
            } catch (Exception $e) {
                return response()->json([
