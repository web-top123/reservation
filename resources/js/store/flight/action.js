...
const actions = {
    ...
    getAircraftFlightsById(context, aircraftFlightId) {
        ApiService.setHeader();
        return new Promise((resolve) =>{
            ApiService.get("api/v1/admin/checkin/aircraft_flight/" + aircraftFlightId)
                .then(({data}) => {
                    context.commit(type.SET_AIRCRAFT_CHECK_FLIGHT, data)
                })
                .catch(({ response }) => {
                    // context.commit(type.AUTH_LOGOUT);
                });
    ...
    updateCheckInFlight(context, checkinFlightInfo) {
        ApiService.setHeader();
        return new Promise((resolve, reject) => {
            ApiService.put("api/v1/admin/checkin/flight/update", checkinFlightInfo)
                .then((data) => {
                    resolve(data);
                })
                .catch(({response, status}) => {
